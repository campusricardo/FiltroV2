...
const postCamper = async(req,res) => {
    const {nombre, tipoIdentificacion, NroIdentificacion, email, password, level, levelState, estado, rol, promedio} = req.body;
    console.log(req.files | "xd");
    const error = {};
try {
    ...
    const data = {
        ...
        imagen: !req.files || Object.keys(req.files).length === 0 || !req.files.image ? error.imagen = 'There is no image': imagen  ,
